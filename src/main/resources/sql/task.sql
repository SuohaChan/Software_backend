...
-- 插入任务数据，所有任务都关联到默认教师（ID=1）
INSERT INTO `tb_task` (
    `label`, `score`, `top`, `selected`, `title`,
    `desc`, `level`, `college`, `clazz`, `deadline`,
    `create_time`, `update_time`, `is_published`, `teacher_id`
) VALUES
      ('日常任务', 5, 0, 0, '计算机学院卫生检查', '对各班级卫生情况进行检查评分', '班级', '计算机科学与技术学院', '软件2201', '2025-08-10 23:59:59', '2025-08-01 09:30:00', '2025-08-01 09:30:00', 1, 1),
      ('重要通知', 10, 1, 1, '全校安全教育讲座报名', '参与讲座可获得综合素质学分', '校级', '', '', '2025-09-15 18:00:00', '2025-08-01 10:15:00', '2025-08-01 10:15:00', 1, 1),
      ('作业', 3, 0, 0, '高等数学课后作业提交', '完成教材第5章习题', '班级', '数学与统计学院', '数学一班', '2025-08-05 22:00:00', '2025-08-01 14:20:00', '2025-08-01 14:20:00', 0, 1),
      ('活动', 8, 0, 1, '外语学院演讲比赛', '主题：我的大学生活', '院级', '外国语学院', '', '2025-08-08 17:00:00', '2025-08-01 15:00:00', '2025-08-01 16:30:00', 1, 1),
      ('调研', 15, 1, 0, '大学生消费习惯调查', '提交调查问卷可参与抽奖', '校级', '', '', '2025-08-20 20:00:00', '2025-07-28 08:45:00', '2025-07-29 11:20:00', 1, 1),
      ('会议', 0, 0, 0, '学院教学工作会议', '各系主任参加，讨论新学期教学计划', '院级', '机械工程学院', '', '2025-08-12 09:00:00', '2025-08-01 11:30:00', '2025-08-01 11:30:00', 0, 1),
      ('签到', 1, 0, 0, '班主任班会签到', '周五下午班会现场签到', '班级', '经济管理学院', '国贸二班', '2025-08-02 17:30:00', '2025-08-01 08:10:00', '2025-08-01 08:10:00', 1, 1),
      ('竞赛', 20, 1, 1, '大学生创新创业大赛', '提交创业计划书，优胜者推荐参加省赛', '校级', '', '', '2025-08-30 00:00:00', '2025-07-25 16:40:00', '2025-07-26 09:15:00', 1, 1),
      ('实践', 6, 0, 0, '暑期社会实践报告', '提交3000字以上实践报告', '班级', '人文学院', '汉语言文学三班', '2025-08-18 22:00:00', '2025-08-01 13:50:00', '2025-08-01 13:50:00', 0, 1),
      ('志愿', 4, 0, 1, '校园迎新志愿者招募', '帮助新生办理入学手续', '院级', '医学院', '', '2025-08-25 12:00:00', '2025-08-01 10:00:00', '2025-08-01 10:00:00', 1, 1),
      ('实践', 6, 0, 0, '原神', '提交实践报告', '校级', '', '', '2025-08-18 22:00:00', '2025-08-11 13:50:00', '2025-08-01 13:50:00', 1, 1);
...
SELECT
    ...
    NOW() as update_time
FROM tb_task t
         INNER JOIN tb_student_class sc ON t.college = sc.college
WHERE t.level = '院级'
  AND t.is_published = 1
  AND NOT EXISTS (
    SELECT 1 FROM tb_student_task st
...
SELECT
    ...
    NOW() as update_time
FROM tb_task t
         INNER JOIN tb_student_class sc
                    ON t.college = sc.college
                        AND t.clazz = sc.clazz
WHERE t.level = '班级'
  AND t.is_published = 1
  AND NOT EXISTS (
    SELECT 1 FROM tb_student_task st
